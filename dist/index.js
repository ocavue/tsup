...
function formatDistributionExports(exports, fromFilePath, toFilePath) {
  ...
  }).map((declaration) => formatDistributionExport(declaration, importPath)).filter(_chunkBL2PMODFjs.truthy);
  if (lines.length === 0) {
    lines.push("export {};");
  }
  return lines.join("\n") + "\n";
...
async function build(_options) {
  ...
  await Promise.all(
    [...Array.isArray(configData) ? configData : [configData]].map(
      async (item) => {
        const logger3 = _chunkX22RQBZHjs.createLogger.call(void 0, _optionalChain([item, 'optionalAccess', _31 => _31.name]));
